...
def insert_emp(request):
    if request.method == "POST":
        ...
        data = Employee1(EmpId=EmpId, EmpName=EmpName, EmpGender=EmpGender, EmpEmail=EmpEmail,
...
def edit_emp(request,pk):
    employees = Employee1.objects.get(id=pk)
    if request.method == 'POST':
            print(request.POST)
...
def remove_emp(request, pk):
    try:
        employee = get_object_or_404(Employee1, id=pk)
